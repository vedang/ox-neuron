...
a index/ folder."
  (let* ((filename (org-string-nw-p
         ...
         (slug (or (org-element-property :EXPORT_HUGO_SLUG entry)
                   (org-hugo-get-heading-slug entry '())))
         (zfilename (string-limit (concat zettel-id "-" slug) 32)))
    ;; Store the id to slug mapping for later usage
    (puthash zettel-id (or filename zfilename) org-neuron-id-to-slug-hash)
    ;; This hacky cond because Neuron cannot handle an index.md file
    ;; as well as a index/ folder.
    (cond
     ((and (equal "index" filename)
      ...
      zettel-id)
     (filename
      filename) ; manually set filename is always preferred.
     (org-neuron-export-human-readable-filenames
      zfilename) ; human-readable, generated name should be used if
                ; user asks for it.
     (t zettel-id)))) ; else use the zettel-id
